...
export default NavBar;
      ...
      {/* Mobile Menu */}
      {menuOpen && user && (
        <div className="md:hidden bg-white/95 backdrop-blur-md border-t border-neutral-100 animate-slide-up">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              onClick={() => setMenuOpen(false)}
              className="flex items-center gap-3 w-full px-4 py-3 text-neutral-700 hover:bg-neutral-50 border-b border-neutral-100 transition-colors"
            >
